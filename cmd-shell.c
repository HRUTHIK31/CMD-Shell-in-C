...
{ 
 ...
 printf("\n\n\nUSER is: \e[1;31m%s\e[0m",username);  printf("\n"); 
} 
//global variables 
int pipe_count=0, fd; 
...
static int split(char *cmd_exec, int, int, int); static int command(int, int, int, char *cmd_exec); 
...
while (*ptr) {
 ...
 for (i = j-1; val!=0; i--) 
 { 
 printf ("%d: %s\n", i + history_base, history[i]->line);  val--; 
 } 
 } 
} 
///------------/// 
...
{ 
 ...
 { 
 num_ch[0]=strtok(input_buffer,"!");  num = atoi(num_ch[0]); 
 strcpy(cmd_val, history_data[num-1]);  } 
 tokenise_cmd[0]=strtok(cmd_val," ");  while ((tokenise_cmd[n]=strtok(NULL,""))!=NULL)  n++; 
 tokenise_cmd[n]=NULL; 
...
if(args[1]==NULL) 
 chdir(h); 
else if ((strcmp(args[1], "~")==0) || (strcmp(args[1], "~/")==0))  chdir(h); 
...
//splits the command and sends to the command function static int split(char *cmd_exec, int input, int first, int last) { 
 char *new_cmd_exec1; 
 new_cmd_exec1=strdup(cmd_exec); 
 //else 
 { 
 ...
 { 
 cmd_exec = skip_comma(new_cmd_exec1);  int m=1; 
 args[0]=strtok(cmd_exec," ");  while((args[m]=strtok(NULL," "))!=NULL)  m++; 
 args[m]=NULL; 
 } 
...
//if there is pipe requested this function will be useful // this function is responsible for checking the command after the initial processing and passes the output to split()
...
{ 
 int status; 
 char ch[2]={"\n"}; 
 getcwd(current_directory, sizeof(current_directory));  signal(SIGINT, sigintHandler); 
